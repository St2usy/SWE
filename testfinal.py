...
class calculator:
    ...
    def run_calculator(self):
        first_operand = self.check_not_integer(input())
    ...
    def __calculate_normal(self, first_operand, operator):
        while operator != "=":
            next_operand = self.check_not_integer(input())
            ...
            self.check_operator_same(operator, next_operator)
    ...
    def __factorial(self, number):
        if self.check_factorial_valid(number):
    ...
    # 연산자 연속성 체크
    def check_operator_same(self, operator, next_operator):
    ...
    # 팩토리얼 피연산자 확인
    def check_factorial_valid(self, number):
        if number < 0:
...
class Test_check_not_integer(unittest.TestCase): # 정수가 아닌 피연산자 체크
    def test2(self):
        ...
        result = calc.check_not_integer('2')
        result = calc.check_not_integer('0.01')
        result = calc.check_not_integer('+')
        result = calc.check_not_integer('0')
        result = calc.check_not_integer('-1')
...
class Test_check_factorial_valid(unittest.TestCase): # 팩토리얼 피연산자 확인
    def test3(self):
        ...
        result = calc.check_factorial_valid(1)
        result = calc.check_factorial_valid(100)
        result = calc.check_factorial_valid(-1)
        result = calc.check_factorial_valid(-15)
        result = calc.check_factorial_valid(-27)
        self.assertEqual(calc.get_errno(), [4,4,4])
